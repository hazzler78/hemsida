...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!SUPABASE_URL || !SUPABASE_KEY) {
      console.error('Supabase env saknas för banner-click', {
    ...
    const { variant, href, sessionId } = body || {};
    ...
    if (error) {
      console.error('Supabase banner_clicks insert error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    return NextResponse.json({ ok: true });
  } catch (e: unknown) {
    const message = e instanceof Error ? e.message : 'Okänt fel';
    console.error('Banner-click endpoint error:', message, e);
